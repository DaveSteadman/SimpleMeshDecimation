...
public static class FssXYPolyLineOperations
{
    // --------------------------------------------------------------------------------------------
    // 3 point Bezier curve
    // --------------------------------------------------------------------------------------------

    public static FssXYPoint Calculate3PointBezierPos(double t, FssXYPoint start, FssXYPoint control, FssXYPoint end)
    {
        double u = 1 - t;
        double tt = t * t;
        double uu = u * u;
    ...
    public static double Calculate3PointBezierTangentAngle(double t, FssXYPoint start, FssXYPoint control, FssXYPoint end)
    {
    ...
    public static FssXYPoint Calculate3PointBezierFirstDerivative(FssXYPoint start, FssXYPoint control, FssXYPoint end, double t)
    {
        // Calculate the components of the derivative
        double dx = 2 * (1 - t) * (control.X - start.X) + 2 * t * (end.X - control.X);
        double dy = 2 * (1 - t) * (control.Y - start.Y) + 2 * t * (end.Y - control.Y);

        // Return the derivative as a FssXYPoint
        return new FssXYPoint(dx, dy);
    }

    // Calculate the second derivative for a 3-point Bezier curve
    public static FssXYPoint Calculate3PointBezierSecondDerivative(FssXYPoint start, FssXYPoint control, FssXYPoint end, double t)
    {
        // For a quadratic Bezier curve, the second derivative is constant
        return new FssXYPoint(2 * (end.X - 2 * control.X + start.X), 2 * (end.Y - 2 * control.Y + start.Y));
    }

    public static FssXYPolyLine? Create3PointBezier(FssXYPoint p1, FssXYPoint p2, FssXYPoint p3, int numSegments)
    {
        FssXYPolyLine? bezier = null;

        if (numSegments > 0)
        {
            double t = 0;
            double dt = 1.0 / numSegments;
            FssXYPoint[] points = new FssXYPoint[numSegments + 1];

            for (int i = 0; i <= numSegments; i++)
            {
                double fraction = (double)i / numSegments;

                points[i] = FssXYPolyLineOperations.Calculate3PointBezierPos(fraction, p1, p2, p3);
                t += dt;
            }

            bezier = new FssXYPolyLine(new System.Collections.Generic.List<FssXYPoint>(points));
        }

        return bezier;
    }

    public static double Calculate3PointBezierCurvature(double t, FssXYPoint start, FssXYPoint control, FssXYPoint end)
    {
    ...
    public static FssXYPoint Calculate4PointBezierPos(double fraction, FssXYPoint start, FssXYPoint control1, FssXYPoint control2, FssXYPoint end)
    {
        double u = 1 - fraction;
    ...
    public static double Calculate4PointBezierTangentAngle(double t, FssXYPoint start, FssXYPoint control1, FssXYPoint control2, FssXYPoint end)
    {
    ...
    public static FssXYPoint Calculate4PointBezierFirstDerivative(FssXYPoint start, FssXYPoint control1, FssXYPoint control2, FssXYPoint end, double t)
    {
        double dx = 3 * (1 - t) * (1 - t) * (control1.X - start.X) +
                    6 * (1 - t) * t * (control2.X - control1.X) +
                    3 * t * t * (end.X - control2.X);
    ...
    public static FssXYPoint Calculate4PointBezierSecondDerivative(FssXYPoint start, FssXYPoint control1, FssXYPoint control2, FssXYPoint end, double t)
    {
    ...
    public static FssXYPolyLine? Create4PointBezier(FssXYPoint p1, FssXYPoint p2, FssXYPoint p3, FssXYPoint p4, int numSegments)
    {
        FssXYPolyLine? bezier = null;
    ...
    // CalculateCurvature now takes FssXYPoint arguments for first and second derivatives
    ...
    private static double CalculateCurvature(FssXYPoint firstDerivative, FssXYPoint secondDerivative)
    {
